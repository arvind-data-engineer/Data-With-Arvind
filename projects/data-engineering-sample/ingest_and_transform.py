...
REQUIRED_COLUMNS = ['order_id', 'order_date', 'product_id', 'customer_id', 'quantity', 'unit_price']
...
def validate_sales(sales: pd.DataFrame) -> pd.DataFrame:
    ...
    if not duplicate_orders.empty:
        raise ValueError(f"Duplicate order_id values found: {duplicate_orders['order_id'].tolist()}")
...
def transform_sales(sales: pd.DataFrame, products: pd.DataFrame) -> pd.DataFrame:
    ...
    fact_sales = sales[
        ['order_id', 'order_date', 'order_month', 'order_day', 'customer_id', 'product_id', 'product_name', 'product_category', 'quantity', 'unit_price', 'revenue']
    ]
...
def save_outputs(fact_sales: pd.DataFrame, daily_summary: pd.DataFrame):
